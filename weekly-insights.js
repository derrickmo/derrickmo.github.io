        ...
        items: [
          {
            ...
            whatsNew: "On September 8, 2026, OpenAI published an analytical proof plus a Lean formalization showing the 3D incompressible Navier-Stokes equations can develop a finite-time singularity from smooth initial data under smooth forcing with finite energy, resolving statements C and D of the Clay Millennium Prize formulation. It was produced by a multi-agent system running on an internal model OpenAI describes as significantly more capable than GPT-6 Astra, in training since August 28. A September 10 update added findings from an investigation into whether user inputs could have influenced the result.",
            howItWorks: "The orchestration is the part that generalizes, not the mathematics. Agents were split into communicating groups; the group that produced the result ran on the order of 10,000 concurrent agents, each with tool access to a cached copy of the internet and a code interpreter. Different groups were prompted with contradictory variants of the problem statement (proof variants A and B, disproof variants C and D), so the swarm hedged across outcomes instead of committing to one. When groups plateaued, Codex was used to consolidate the most useful intermediate results across groups and the consolidation was fed back as follow-up prompts, cross-pollinating otherwise isolated search trees. An easier warmup problem, unforced Euler regularity, was resolved first by about 100 agents over roughly 50 hours and then injected into the Navier-Stokes prompts as a seed. Lean is a proof assistant, so a formalization is machine-checkable and does not depend on human referees catching errors.",
            impact: "The published token economics are the most reusable artifact. Across all attempted problems the agents sent 4.9 million messages and consumed roughly 300 billion output tokens; Navier-Stokes alone took 2.7 million messages and about 130 billion output tokens over 88 hours, with 17 more hours of GPT-6 Astra for Lean verification. At any public frontier output price that is a seven-figure inference bill for one result, which sets a realistic floor on what throwing a swarm at a problem costs. Three design choices transfer to ordinary agent systems: prompt disjoint groups with contradictory hypotheses rather than one shared objective; score with a cheap verifier the agents never see (Lean here) instead of trusting swarm confidence; and periodically consolidate cross-group intermediate results with a separate model rather than letting groups run isolated to completion. Treat the result itself as unrefereed: Alpoge (Anthropic) and Buckmaster (NYU) independently produced a Lean-verified forced-Euler blowup with an internal Anthropic model, and a Caltech/NVIDIA group posted an unforced self-similar Euler stability framework on Sept 9 (arXiv 2609.10860).",
          ...
          },
          {
            ...
            impact: "The practical win is latency and cost structure as much as accuracy. Sequential memory agents are serial by construction: segment N+1 cannot start until segment N's memory update lands, so wall-clock time scales linearly with document length and you cannot batch. ConvMem parallelizes across both segments at a level and kernels within a level, so the work maps onto vLLM or SGLang as one wide batch per tree level with only log(N) synchronization barriers. Being training-free, it costs nothing to try against an existing endpoint, which is the relevant comparison against MemAgent-style approaches that need an RL run before you know whether they help on your data. The trade is total token spend: strided overlap plus multiple kernels reads more tokens than one linear pass, so this buys latency and robustness with tokens. Worth benchmarking if you run multi-hop retrieval over documents that exceed your context window and your current answer is to chunk it and hope.",
          ...
          },
          {
            ...
            whatsNew: "Submitted September 5, 2026, two days outside this week's window, and included because it lands squarely on RL post-training data selection and was not covered in last week's digest. DataFlex-RL (arXiv 2609.06107) is a controlled platform comparing RLVR data policies under one fixed GRPO recipe. Across 13 configurations and 12 matched seeds on Qwen2.5-7B-Base over 12 math, logic, and science benchmarks, none of eight rollout-selection or reweighting methods achieved a paired 95% confidence interval excluding zero relative to plain uniform sampling, and none of three adaptive domain mixtures beat a fixed equal mixture at the same precision. A 12-seed extension on Llama-3.1-8B-Base found no consistent winner either.",
            howItWorks: "RLVR is reinforcement learning from verifiable rewards, where a program checks the answer instead of a learned reward model scoring it. A data policy in this setting decides which sampled rollouts enter the gradient, how much weight each gets, and how much of each domain appears in the next batch. GRPO (Group Relative Policy Optimization) computes advantages by comparing responses within a sampled group rather than against a learned value critic, which is why it needs no critic network. The contribution is methodological control: identical GRPO hyperparameters, base checkpoint, and benchmark suite, 12 matched seeds per configuration, and paired confidence intervals instead of single-seed point comparisons. Uniform GRPO itself works fine, lifting domain-balanced average accuracy 7.76 percentage points over the untrained checkpoint. The finding is that the clever selection layer on top adds nothing measurable.",
            impact: "Two things change if you build post-training pipelines. First, deprioritize rollout-filtering and reweighting engineering: if you planned difficulty-based filtering, advantage-magnitude reweighting, or an adaptive domain scheduler on top of GRPO, spend that time on reward quality, verifier coverage, or rollout count instead, and hold uniform sampling as a baseline you must actually beat with paired multi-seed intervals. Second, and more broadly useful, the paper quantifies benchmark-suite sensitivity by rescoring nine Qwen2.5-7B-Instruct runs under a math-heavy six-benchmark summary (five math benchmarks plus GPQA-Diamond, no logic benchmark) against the domain-balanced 12-benchmark summary. The two rankings are negatively correlated at -0.33. Dropping one domain from your eval suite can invert which checkpoint you ship. Fix the eval suite before comparing training recipes, and report seeds.",
        ...
        items: [
          {
            ...
            whatsNew: "vLLM v0.29.0 shipped September 9, 2026 with 594 commits from 277 contributors. Model Runner V2 is now the default for all models, completing a rollout that began with pooling models, and MRV1 is formally deprecated with removal targeted for v0.32. Ten deprecated architectures were removed (Arctic, Chameleon, Cheers, Fairseq2Llama, FireRedLID, GritLM, HCXVision, MPT, the RWForCausalLM and StableLMEpochForCausalLM aliases, and PrithviGeoSpatialMAE). FlexOlmo, Olmo3 and Hunyuan V1/VL now route through the Transformers modeling backend. The PyAV video decoder backend is gone; use OpenCV or Torchcodec. The python -m vllm.entrypoints.openai.api_server entrypoint is deprecated in favor of vllm serve.",
            howItWorks: "The model runner takes a scheduled batch, lays out KV cache and attention metadata, runs the forward pass, and samples. MRV2 is the rewrite. Alongside becoming default it gained CUDA graph memory profiling for KV cache auto-sizing, batch-sharded sampling that cuts per-step logits memory by 1/TP (an 8-way tensor-parallel deployment holds one eighth the logits per step, which matters at large vocab and large batch), prompt embeds, extract_hidden_states speculation, padded FULL cudagraph dispatch for uniform decode under speculative decoding, and DP-sync skipping before EAGLE/MTP draft prefill. EAGLE and MTP are speculative decoding schemes where a small draft head proposes several tokens the full model then verifies in one pass. Separately, Mamba prefix caching gained internal prefill checkpoints worth a 9% to 25% TTFT improvement on hybrid state-space models.",
            impact: "Test this one carefully rather than rolling forward blind. MRV2 does not yet support sequence parallelism, dual-batch overlap, elastic expert parallelism, custom logits processors, or certain speculative decoding methods; vLLM falls back to MRV1 automatically when those are configured and expects to close the gaps in two to three weeks. That silent fallback is the thing to check, because if you configure one of those features you are quietly still on a runner slated for removal in three releases, and MRV1-specific optimizations are no longer being accepted. Before upgrading: confirm your architecture is not among the removed ten, swap PyAV for Torchcodec if you decode video, change any python -m vllm.entrypoints.openai.api_server invocation to vllm serve, and log which runner actually initialized. If you serve Mamba or hybrid models the prefix-caching checkpoints are a free 9-25% TTFT win, but note that prefix_cache_retention_interval changed default from dense to 0 for SWA and SSM models and is now a CLI argument rather than an env var.",
