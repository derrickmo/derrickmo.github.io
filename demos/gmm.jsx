...
function GMMDemo() {
  ...
  const explainer = (
    <>
      <DemoP>
        ...
        point is generated by one of K Gaussians, but we do not know which. EM solves
        the chicken-and-egg problem in two repeating steps.
      </DemoP>
      <DemoUL>
        <DemoLI>
          The <b>E-step</b> computes the <i>responsibility</i> of each point, the
          probability it belongs to each component. Its color here is those
          probabilities blended together.
        </DemoLI>
        <DemoLI>
          The <b>M-step</b> re-fits the mean, covariance and weight of every Gaussian
          using those soft assignments.
        </DemoLI>
        <DemoLI>
          <b>Step EM</b> once at a time, or <b>Run</b> to watch the ellipses snap onto
          the clusters. Each step is guaranteed not to decrease the log-likelihood.
        </DemoLI>
      </DemoUL>
      <DemoP>
        Unlike k-means, GMM captures <i>elongated, rotated</i> clusters because each
        component carries a full covariance matrix, and the same ellipse geometry
        comes straight from its 2&times;2 eigen-decomposition. This soft-assignment
        idea is the backbone of mixture models, speaker clustering and the variational
        methods behind modern generative models.
      </DemoP>
    </>
  );
  ...
  const concepts = (
    <>
      <DemoP>
        GMMs are the canonical soft-clustering and density-estimation model: speaker
        and audio clustering, background subtraction in vision, anomaly detection
        where low-density points are the outliers, and a flexible distribution you can
        sample from. Because each component carries a full <i>covariance</i>, GMMs
        capture elongated, correlated, rotated clusters that k-means, with its round
        hard cells, cannot.
      </DemoP>
      <DemoP>
        The deeper prize is <b>EM</b> itself, alternating between soft assignments and
        parameter updates to maximize a likelihood you cannot optimize directly. That
        exact pattern shows up three more times:
      </DemoP>
      <DemoUL>
        <DemoLI>The "responsibility" framing is a latent variable by another name.</DemoLI>
        <DemoLI>
          It is the conceptual ancestor of variational inference and the ELBO
          objective.
        </DemoLI>
        <DemoLI>
          That objective is what trains VAEs and other modern latent-variable
          generative models.
        </DemoLI>
      </DemoUL>
    </>
