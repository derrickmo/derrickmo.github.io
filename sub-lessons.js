...
// Pages are generated by _private/scripts/gen-sublesson-pages.mjs.
      ...
      "lora": {
        ...
        "sections": [
          ...
          },
          {
            "h": "The math",
            "paras": [
              "Add a rank-r update B A to the frozen weight W:"
            ],
            "tex": "W' = W + B A,\\qquad B\\in\\mathbb{R}^{d\\times r},\\; A\\in\\mathbb{R}^{r\\times k},\\; r\\ll d",
            "texNote": "Only A and B are trained; W stays frozen. Tiny, swappable, and mergeable."
          },
          {
            "h": "In code",
            "code": "# only A, B are trainable; W is frozen\nA = np.random.randn(r, k) * 0.01\nB = np.zeros((d, r))\ny = x @ (W + B @ A).T          # forward uses W + BA",
            "caption": "A few million low-rank parameters specialize a frozen model."
          }
        ],
        "takeaways": [
          "LoRA learns a low-rank correction to frozen weights.",
          "It cuts trainable parameters and memory by orders of magnitude.",
          "Adapters are swappable and mergeable per task."
        ],
        "demo": "lora"
      },
      "reward-model": {
      ...
      "mixed-precision": {
        ...
        "sections": [
          ...
          },
          {
            "h": "The math",
            "paras": [
              "Loss scaling shifts gradients into fp16's representable range:"
            ],
            "tex": "g = \\tfrac{1}{S}\\,\\nabla(S\\cdot\\mathcal{L})",
            "texNote": "Scale up by S before backward, unscale by S after - bf16's wider range often skips this."
          },
          {
            "h": "In code",
            "code": "scaled_loss = loss * S            # S ~ 2^14\nscaled_loss.backward()\ngrads = [g / S for g in grads]    # unscale before the step",
            "caption": "Compute in 16-bit, accumulate the master weights in 32-bit."
          }
        ],
        "takeaways": [
          "fp16 / bf16 cut memory and speed up training.",
          "Loss scaling stops fp16 gradients underflowing.",
          "An fp32 master copy preserves accuracy."
        ],
