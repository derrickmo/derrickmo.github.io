...
// The 502-entry destination list lives in public/search-index.js and is fetched on
// first palette open - it is ~140 KB of pure data and this file loads on every page.
...
// The destination list is ~140 KB of pure data and this file loads on every page, so it
// is fetched on first palette open rather than shipped site-wide (PF-0021). Injected as a
// script because the generators emit it as a JS array literal; it sets window.DM_NAV_INDEX.
let __navPending = null;
const __navWaiters = [];
function loadNavIndex() {
  if (typeof window === "undefined" || window.DM_NAV_INDEX) return;
  if (__navPending) return;
  __navPending = document.createElement("script");
  __navPending.src = `${window.__DM_BASE || "/"}search-index.js`;
  __navPending.onload = () => { __navWaiters.splice(0).forEach(fn => fn()); };
  // A failed load leaves the palette empty rather than broken; allow a later retry.
  __navPending.onerror = () => { __navPending = null; __navWaiters.splice(0).forEach(fn => fn()); };
  document.head.appendChild(__navPending);
}
function onNavIndexReady(fn) {
  if (typeof window !== "undefined" && window.DM_NAV_INDEX) { fn(); return; }
  __navWaiters.push(fn);
}

function paletteFilter(q) {
  const all = ((typeof window !== "undefined" && window.DM_NAV_INDEX) || []).concat(__conceptEntries());
...
function CommandPalette({ open, setOpen }) {
  ...
  // Bumped when the lazily-fetched index arrives, so results re-render without it
  // needing to be in state (the array is large and never mutates).
  const [, setNavTick] = __useState(0);

  __useEffect(() => {
    if (open) {
      loadNavIndex();
      onNavIndexReady(() => setNavTick(t => t + 1));
      setQ(""); setSel(0);
  ...
  const indexReady = typeof window !== "undefined" && !!window.DM_NAV_INDEX;
    ...
    <div role="dialog" aria-label="Command palette" onClick={() => setOpen(false)}
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ maxHeight: "52vh", overflowY: "auto", padding: 6 }}>
          {results.length === 0 && (
            <div className="t-body" style={{ color: "var(--muted)", padding: "20px 14px", fontSize: 14 }}>
              {indexReady ? "No matches." : "Loading index…"}
            </div>
          )}
          {results.map((it, i) => (
          ...
          {mobile ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <button onClick={() => setCmdOpen(true)} aria-label="Search" className="t-mono-s"
                ...
                onMouseEnter={e => { loadNavIndex(); e.currentTarget.style.color = "var(--blue-br)"; e.currentTarget.style.borderColor = "var(--blue-lt)"; }}
                onMouseLeave={e => { e.currentTarget.style.color = "var(--muted)"; e.currentTarget.style.borderColor = "var(--border)"; }}
                onFocus={loadNavIndex}>
